...
int main() {
    ...
    array<DecodePlan, 21> plans;

    auto estimate_plan = [&](double alpha) -> DecodePlan {
        DecodePlan best;

        const double base_pref_excess =
            max(0.0, worst_pref_ref / max(SLO1, 1e-12) - 1.0);
        ...
        for (int B = 1; B <= MAXR; ++B) {
            ...
            // One logical decode cohort. UP and DOWN are independent FIFO links,
            // so each direction is one pipeline resource.
            double edge = dcost[DPRE][B] + dcost[DPOST][B];
            double up = m * latency_ms + wire_one * B;
            double down = up;

            // All remotes run in parallel. The largest assigned subgroup defines
            // the cohort's cloud span.
            double cloud = dcost[DPROC][max(1, g)];

            double interval = max({edge, up, down, cloud});
            double rate = B / max(interval, 1e-12);

            // Conservative end-to-end latency for one cohort. The number of
            // cohorts needed to keep the slowest resource continuously occupied
            // is approximately ceil(latency / initiation_interval).
            double latency = dcost[DPRE][B] + up + cloud + down + dcost[DPOST][B];
            int depth = max(1, (int)ceil(latency / max(interval, 1e-12) - 1e-12));
            depth = min(depth, max(1, MAXR / B));

            double tpot_est = depth * interval;
            int window = min(MAXR, max(B, B * depth));

            double ntp = clamp(
                (rate - tp_base) / max(tp_UB - tp_base, 1e-12),
                0.0, 1.0
            );

            double dec_excess =
                max(0.0, tpot_est / max(SLO2, 1e-12) - 1.0);
            double dist_est = hypot(base_pref_excess, dec_excess);

            double nc;
            if (dist_base > 0) {
                nc = clamp(1.0 - dist_est / dist_base, 0.0, 1.0);
            } else {
                nc = dist_est <= 1e-12 ? 1.0 : 0.0;
            }

            double u = alpha * ntp + (1.0 - alpha) * nc;

            // Once two choices have effectively identical score, prefer the
            // smaller working set, then the higher raw rate.
            if (u > best.util + 1e-10 ||
                (abs(u - best.util) <= 1e-10 &&
                 (window < best.window ||
                  (window == best.window && rate > best.rate + 1e-12)))) {
                best.batch = B;
                best.remote_batch = max(1, g);
                best.depth = depth;
                best.window = window;
                best.rate = rate;
                best.tpot = tpot_est;
    ...
    for (int z = 0; z <= 20; ++z) {
        plans[z] = estimate_plan(z / 20.0);
    }

    auto current_plan = [&]() -> const DecodePlan& {
        int z = (int)llround(clamp(sched_wtp, 0.0, 1.0) * 20.0);
        z = max(0, min(20, z));
        return plans[z];
    };

    auto cohort_target = [&]() -> int {
        int b = current_plan().batch;
        double lo = estimated_lout();

        // Hidden Lout is learned only after FIN. For very short outputs a large
        // steady-state cohort never gets reused enough to repay its waiting cost.
        if (finished_request_count >= 4) {
            if (lo <= 1.20) b = min(b, max(2, 2 * K));
            else if (lo <= 2.25) b = min(b, max(4, 4 * K));
            else if (lo <= 4.0) b = min(b, max(8, 8 * K));
        }

        return max(1, min(MAXR, b));
    };

    auto admission_target = [&]() -> int {
        int w = current_plan().window;
        double lo = estimated_lout();

        if (finished_request_count >= 4) {
            if (lo <= 1.20) w = min(w, max(2 * K, 4));
            else if (lo <= 2.25) w = min(w, max(6 * K, 8));
            else if (lo <= 4.0) w = min(w, max(12 * K, 16));
        }

        return max(1, min(MAXR, w));
    };

    auto batch_for = [&](int t, int n) -> int {
        n = max(1, min(MAXR, n));

        // Pure latency mode keeps the completion-time-optimal local policy.
        if (sched_wtp <= 0.15) {
            return bestMeanB[t][n];
        }

        const DecodePlan &p = current_plan();
        int target = (t == DPROC ? p.remote_batch : p.batch);
        target = max(1, min(MAXR, target));

        // If we have not yet accumulated a full planned cohort, process all
        // currently ready work rather than splitting it into even smaller tasks.
        if (n <= target) return n;

        // For strongly throughput-oriented states, the analytical target is the
        // anchor. In mixed states blend it with the old weighted local optimum.
        if (sched_wtp >= 0.55) return min(n, target);

        int local = bestB[t][n];
        int blended = (target + local + 1) / 2;
        return min(n, max(1, blended));
    };
    ...
    auto stable_batch_for = [&](int t, int n) -> int {
        ...
        if (use_aggressive_cohort() && sched_wtp >= 0.72 && n >= 2) {
    ...
    vector<deque<double>> upDecFinish(K);
    ...
    auto enqueue_up = [&](double now, int remote, bool dec, int m, int len) {
        ...
        if (dec) {
            pendingUpDecReq[remote] += m;
            upDecFinish[remote].push_back(fin);
        }
    };
    ...
    auto choose_remote = [&](double now) {
        ...
        for (int k = 0; k < K; ++k) {
            ...
            double score;
            if (sched_wtp >= 0.72) {
                // A balanced long-lived decode population gives every cloud a similarly
                // sized D PROC batch.  Strongly penalize population imbalance first, then
                // use current work as a tie-breaker.
                score = decodeActive[k] * 1000.0 * max(1.0, dcost[DPROC][1])
                      + activeCloud[k] * 80.0 * max(1.0, dcost[DPROC][1])
                      + pendingUpDecReq[k] * dcost[DPROC][1]
                      + rem_busy + 0.25 * prefillLoad[k];
            } else {
    ...
    while (cin >> head) {
        ...
        for (int ev = 0; ev < e; ++ev) {
            ...
            if (kind == "ARR") {
                ...
                if (dir == "UP") {
                    ...
                    if (dec) {
                        pendingUpDecReq[remote] -= m;
                        if (!upDecFinish[remote].empty()) upDecFinish[remote].pop_front();
                    }
                } else {
        ...
        auto decode_window_target = [&]() -> int {
            int t = admission_target();
            if (sched_wtp < 0.20) t = min(t, max(1, K));
            else if (sched_wtp < 0.40) t = min(t, max(2, 2 * K));
            return max(1, min(MAXR, t));
        };
        ...
        if (produced_tokens >= 4 && first_arrival < 1e290 && now > first_arrival + 1e-12) {
            double tp_now = produced_tokens / (now - first_arrival);
        ...
        for (int k = 0; k < K; ++k) {
            ...
            int exact_bubble_le = -1;
            bool wait_known_decode = false;

            if (!hasD && hasP && !upDecFinish[k].empty()) {
                double tdec = upDecFinish[k].front();
                double dt = tdec - now;

                if (dt > 1e-12) {
                    int pid = qPPROC[k].top().second;
                    int ls = req[pid].next_layer;
                    int rem_layers = num_layers - ls;
                    double per_layer = dur(PPROC, req[pid].lin) / max(1, num_layers);

                    int fit = (int)floor((dt - S + 1e-12) / max(per_layer, 1e-18));
                    fit = min(fit, max(0, rem_layers - 1));

                    if (fit >= 1) {
                        exact_bubble_le = ls + fit;
                    } else {
                        // There is a concrete decode transfer already on the wire.
                        // Starting a long unsplittable piece now would *certainly*
                        // delay it; waiting is preferable when TPOT has meaningful
                        // score weight.
                        double one_layer = S + per_layer;
                        if (one_layer > dt + 1e-12 &&
                            (sched_wc >= 0.20 || req[pid].arr + SLO1 > tdec)) {
                            wait_known_decode = true;
                        }
                    }
                }
            }
            ...
            if (chooseD && sched_wtp >= 0.12) {
                ...
                if (!first_token_wait_forbidden &&
                    use_aggressive_cohort() && sched_wtp >= 0.72 &&
                    ...
                    if (n < desired && nx < 1e290) {
                        ...
                        double cap = slack * (0.30 + 0.65 * sched_wtp);
            ...
            if (chooseD) {
                ...
                int ls = req[id].next_layer;
                int le;
                if (exact_bubble_le >= 0) le = exact_bubble_le;
                else if (forced_p_le >= 0) le = forced_p_le;
                else le = choose_piece_end(id, k, now);
