...
int main() {
    ...
    const double sjf_mix = clamp((w_c - 0.50) / 0.50, 0.0, 1.0);
    ...
    auto qkey_ppre = [&](int id) -> double {
        double fifo = req[id].arr + SLO1;
        double sjf = prefill_total_cost(req[id].lin) + req[id].arr * 1e-12;
        return fifo * (1.0 - sjf_mix) + sjf * sjf_mix;
    };
    auto qkey_pproc = [&](int id) -> double {
        double fifo = req[id].arr + SLO1;
        double frac = double(num_layers - req[id].next_layer) / max(1, num_layers);
        double sjf = S + dur(PPROC, req[id].lin) * frac + transfer_time(req[id].lin)
                   + S + dur(PPOST, req[id].lin) + req[id].arr * 1e-12;
        return fifo * (1.0 - sjf_mix) + sjf * sjf_mix;
    };
    auto qkey_ppost = [&](int id) -> double {
        double fifo = req[id].arr + SLO1;
        double sjf = S + dur(PPOST, req[id].lin) + req[id].arr * 1e-12;
        return fifo * (1.0 - sjf_mix) + sjf * sjf_mix;
    };
    ...
    while (cin >> head) {
        ...
        if (produced_tokens >= 32 && first_arrival < 1e290 && now > first_arrival + 1e-12) {
        ...
        for (int k = 0; k < K; ++k) {
            ...
            if (chooseD) {
                int n = min(MAXR, (int)qDPROC[k].size());
                // Only wait when the *next* UP completion really brings decode work to this
                // cloud.  Waiting for unrelated frames was a major source of idle bubbles.
                if (!upQ.empty() && upQ.front().dec && upQ.front().remote == k && deff > now) {
                    int inc = upQ.front().m;
                    double dt = max(0.0, upQ.front().finish - now);
                    int n2 = min(MAXR, n + inc);
                    double separate = dcost[DPROC][max(1, bestB[DPROC][n])]
                                    + dcost[DPROC][max(1, bestB[DPROC][min(MAXR, inc)])];
                    double combined = dcost[DPROC][max(1, bestB[DPROC][n2])];
                    ...
                    // If we are waiting anyway, fill the exact cloud bubble with a non-final
                    // prefill piece.  It cannot delay the decode batch because its TDN itself
                    // wakes us before (or at) the transfer completion.
                    if (wait_for_d && hasP) {
                        int pid = qPPROC[k].top().second;
                        ...
                        span = min(span, rem - 1); // never inject the large prefill DOWN here
                        if (span >= 1) {
                            forced_p_le = ls + span;
                            chooseD = false;
                            wait_for_d = false;
        ...
        if (!busyE) {
            ...
            for (int z = 0; z < (int)cs.size(); ++z) {
                auto c = cs[z];
                if (c.type == DPOST && !downQ.empty() && downQ.front().dec && c.eff > now) {
                    int n = min(MAXR, (int)qDPOST.size());
                    int inc = downQ.front().m;
                    int n2 = min(MAXR, n + inc);
                    double dt = max(0.0, downQ.front().finish - now);
                    double separate = dcost[DPOST][max(1, bestB[DPOST][n])]
                                    + dcost[DPOST][max(1, bestB[DPOST][min(MAXR, inc)])];
                    double combined = dcost[DPOST][max(1, bestB[DPOST][n2])];
                    double gain = max(0.0, separate - combined);
                    double slack_cap = max(0.0, c.eff - now) * (0.08 + 0.50 * sched_wtp);
                    bool can_wait = dt <= gain * (0.35 + 1.25 * sched_wtp) + 1e-12
                                 && dt <= slack_cap + 1e-12;
                    if (can_wait) {
                        // Work-conserving gap fill on E.  Prefer an alternative task that
                        // finishes before the awaited DOWN result, so batching costs no idle time.
                        int alt = -1;
