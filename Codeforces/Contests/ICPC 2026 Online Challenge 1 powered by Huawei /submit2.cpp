...
int main() {
    ...
    auto choose_piece_end = [&](int id, int k, double now) {
        ...
        if (span == rem && rem > 1 && decode_down_pressure() > 0) {
            ...
            if (px > SLO2 * (0.20 + 0.55 * w_tp) && deadline > now + S) {
                span = rem - 1;
            }
        }
        return ls + span;
    ...
    while (cin >> head) {
        ...
        for (int k = 0; k < K; ++k) {
            ...
            if (chooseD) {
                ...
                qPPROC[k].pop();
                int ls = req[id].next_layer;
                int le = choose_piece_end(id, k, now);
                ...
                if (le == num_layers && total_decode_active() > 0 && decode_down_pressure() > 0) {
                    ...
                    double cap = slack * (0.10 + 0.35 * w_tp);
                    if (px > SLO2 * (0.18 + 0.45 * w_tp) && deadline > now &&
                        nx < 1e290 && nx - now <= cap + 1e-12) {
                        qPPROC[k].push({req[id].arr + SLO1, id});
                        continue;
                    }
                }
                req[id].st = PPROC_RUN;
        ...
        if (!busyE) {
            ...
            if (!qPPRE.empty()) {
                ...
                if (eff > now && total_decode_active() > 0) {
                    ...
                    keyv += pressure * (0.18 + 0.72 * w_c) * min(3.0, ratio);
                }
                cs.push_back({keyv, eff, PPRE, 1});
            }
            if (!qPPOST.empty()) {
