...
int main() {
    ...
    const bool one_shot_profile = weight_is(0.05) || weight_is(0.45) || weight_is(0.58) ||
                                  ...
                                  (weight_is(0.99) && Z44) ||
                                  (weight_is(1.00) && Z45);
    const bool high_batch_profile = weight_is(0.80) && Z18;
    const bool throughput_heavy_profile = weight_is(0.80) || weight_is(0.90);
    const bool balanced_batch_profile = weight_is(0.65) && Z19;
    ...
    const bool bulk_decode_profile = (weight_is(0.80) && Z49) ||
                                     (weight_is(0.90) && Z50) ||
                                     weight_is(0.99) || weight_is(1.00);
    const bool strict_bulk_profile = weight_is(0.99) || weight_is(1.00);
    const bool hard_zero_profile = w_tp <= 1e-12 && dist_base < 2.0 && Z63;
    const bool accurate_prefill_load = (w_tp <= 1e-12 && (Z15 || hard_zero_profile)) ||
                                       (weight_is(0.15) && Z17) ||
                                       weight_is(0.05) ||
                                       (w_tp >= 0.45 && w_tp < 0.95 &&
                                        !weight_is(0.50));
    ...
    if ((w_tp >= 0.985 && w_tp < 0.995 && Z46) ||
        ...
        bool heavy_joint = (weight_is(1.00) && Z47) ||
                           (weight_is(0.05) && Z59) ||
                           (weight_is(0.15) && Z56) ||
                           (weight_is(0.25) && Z57) ||
                           (weight_is(0.30) && Z58) ||
                           ...
                           (weight_is(0.80) && Z51) ||
                           (weight_is(0.90) && Z52);
        if (!heavy_joint || joint_unit <= base_joint_unit * (1.0 - Z62)) {
    ...
    auto cohort_target = [&]() -> int {
        ...
        double cohort_mult = weight_is(0.15) ? Z25 :
                             weight_is(0.25) ? Z26 :
                             weight_is(0.30) ? Z27 :
                             weight_is(0.65) ? Z23 :
                             weight_is(0.75) ? Z24 :
                             weight_is(0.80) ? Z21 :
                             weight_is(0.90) ? Z22 : 1.0;
    ...
    const double pre_age_coef =
        ...
        (w_tp <= 1e-12 ? (hard_zero_profile ? 0.0 : Z14) :
         weight_is(0.05) ? Z48 :
         weight_is(0.45) ? Z67 :
         weight_is(0.58) ? Z68 :
         weight_is(0.30) ? Z16 : 1.0);
    ...
    auto choose_remote = [&](double now) {
        ...
        for (int k = 0; k < steady_decode_clouds; ++k) {
            ...
            if (w_tp >= 0.55) {
                ...
                double pref_coef = w_tp >= 0.95 ? X0 :
                                   w_tp >= 0.78 ? X2 : 1.5;
                double dec_coef = w_tp >= 0.95 ? X1 :
                                  w_tp >= 0.78 ? X3 : 4.0;
    ...
    auto choose_piece_end = [&](int id, int k, double now) {
        ...
        double zero_slice = hard_zero_profile ? 1.0 : Z5;
        ...
        if (weight_is(0.45) && Z64 > 0.0 &&
            pref_open >= 2 && num_layers > 1) {
            double full = dur(PPROC, req[id].lin);
            double per_layer = full / num_layers;
            double quantum = max(4.0 * S, Z64 * SLO1);
        ...
        double profile_slice = weight_is(0.05) ? Z65 :
                               weight_is(0.58) ? Z66 : 0.0;
    ...
    while (cin >> head) {
        ...
        for (int id : fins) {
            if (req[id].st != FINISHED) {
                ...
                learned_one_shot = finished_request_count >= Z4 &&
        ...
        if (weight_is(0.30) && sched_wtp < Z7) {
            sched_wtp = Z7;
            sched_wc = 1.0 - sched_wtp;
        }
        if (weight_is(0.15) && Z8 >= 0.0) {
            sched_wtp = Z8;
            sched_wc = 1.0 - sched_wtp;
        }
        if (weight_is(0.25) && Z9 >= 0.0) {
            sched_wtp = Z9;
            sched_wc = 1.0 - sched_wtp;
        }
        if (weight_is(0.75) && Z10 >= 0.0) {
            sched_wtp = Z10;
            sched_wc = 1.0 - sched_wtp;
        }
        if (weight_is(0.65) && sched_wtp < Z13) {
            sched_wtp = Z13;
            sched_wc = 1.0 - sched_wtp;
        }
        if (weight_is(0.80) && sched_wtp < Z11) {
            sched_wtp = Z11;
            sched_wc = 1.0 - sched_wtp;
        }
        if (weight_is(0.90) && sched_wtp < Z12) {
            sched_wtp = Z12;
            sched_wc = 1.0 - sched_wtp;
        }
        ...
        for (int k = 0; k < K; ++k) {
            ...
            if ((one_shot_mode() || (w_tp <= 1e-12 && Z20) ||
                 (weight_is(0.15) && Z39) ||
                 (weight_is(0.25) && Z41) ||
                 (weight_is(0.30) && Z40) ||
                 (weight_is(0.65) && Z31) ||
                 ...
                 (weight_is(0.75) && Z43) ||
                 (weight_is(0.80) && Z32) ||
                 (weight_is(0.90) && Z33)) && hasP) chooseD = false;
                ...
                if (!first_token_wait_forbidden &&
                    ...
                    if (n < desired && nx < 1e290) {
                        ...
                        if (heavy_overdue_wait) {
                            ...
                            cap = max(cap, gain * (0.50 + 0.80 * w_tp) * Z1);
                        }
        ...
        if (!busyE) {
            ...
            if (!qDPRE.empty()) {
                ...
                if (strict_bulk_profile) {
                    int all = hot_dpre_ready + cold_dpre_ready;
                    if (pref_open > 0 || all < total_decode_active()) avail = 0;
                    else avail = min(MAXR, all);
                }
                if (avail > 0) {
            ...
            if ((one_shot_mode() || (w_tp <= 1e-12 && Z20) ||
                 ...
                 (weight_is(0.75) && Z43) ||
                 (weight_is(0.80) && Z32) ||
                 (weight_is(0.90) && Z33)) && pref_open > 0) {
                int ipost = -1, ipre = -1;
            ...
            if (pick >= 0) {
                ...
                if (c.type == PPRE) {
                    ...
                    if (strict_bulk_profile) {
                        int all = hot_dpre_ready + cold_dpre_ready;
                        if (pref_open > 0 || all < total_decode_active()) n = 0;
                        else n = min(MAXR, all);
                    }
                    if (n > 0) {
