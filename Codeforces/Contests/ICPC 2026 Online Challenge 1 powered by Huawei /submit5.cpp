...
int main() {
    ...
    // Among already-arrived requests, shortest remaining prefill work is an
    // excellent default: it lowers mean TDR while keeping total machine work
    // essentially unchanged.  A small arrival-age term prevents pathological
    // starvation when arrivals are spread out.
    ...
    while (cin >> head) {
        ...
        const bool throughput_gate =
            sched_wtp >= 0.62 && long_decode && decode_now >= admit_goal;
        ...
        for (int k = 0; k < K; ++k) {
            ...
            if (hasD) {
                ...
                dkey = norm_key(deff, now, SLO2, 1.0) - 1.35 * dec_bias;
            }
            if (hasP) {
            ...
            if (chooseD && sched_wtp >= 0.12) {
                ...
                if (sched_wtp >= 0.72 && pendingUpDecReq[k] > 0 && deff > now) {
                ...
                } else if (!upQ.empty() && upQ.front().dec && upQ.front().remote == k && deff > now) {
            ...
            if (chooseD) {
                ...
                if (throughput_gate && le == num_layers && num_layers > 1) {
                    // Keep the final layer as an admission valve.  Non-final chunks use
                    // otherwise idle cloud time without injecting a large prefill DOWN.
                    le = max(ls + 1, num_layers - 1);
                    if (le <= ls) le = num_layers;
                }
        ...
        if (!busyE) {
            ...
            if (!qPPRE.empty() && !throughput_gate) {
            ...
            if (!qDPRE.empty()) {
                ...
                cs.push_back({norm_key(eff, now, SLO2, 0.80) - 1.20 * dec_bias,
            ...
            if (!qDPOST.empty()) {
                ...
                double eff = req[id].decode_due - rem;
                cs.push_back({norm_key(eff, now, SLO2, 1.00) - 1.55 * dec_bias,
            ...
            if (pick >= 0) {
                ...
                if (c.type == PPRE) {
                    ...
                    if (n > 0) {
                        ...
                        if (sched_wtp >= 0.72) b = min(n, max(b, min(n, cohort_target())));
                        vector<int> ids;
                        ids.reserve(b);
                        for (int z = 0; z < b; ++z) {
                            clean(qDPRE, DPRE_READY, -1);
