...
#endif
#ifndef TUNE_065_COHORT_MULT
#define TUNE_065_COHORT_MULT 128.0
#endif
#ifndef TUNE_075_COHORT_MULT
#define TUNE_075_COHORT_MULT 64.0
#endif
#ifndef TUNE_015_COHORT_MULT
#define TUNE_015_COHORT_MULT 1.0
#endif
#ifndef TUNE_025_COHORT_MULT
#define TUNE_025_COHORT_MULT 64.0
#endif
#ifndef TUNE_030_COHORT_MULT
#define TUNE_030_COHORT_MULT 1.0
#endif
...
#define TUNE_099_ONESHOT 0
#endif
#ifndef TUNE_100_ONESHOT
#define TUNE_100_ONESHOT 0
#endif
#ifndef TUNE_099_JOINT
#define TUNE_099_JOINT 1
#endif
#ifndef TUNE_100_JOINT
#define TUNE_100_JOINT 1
#endif
#ifndef TUNE_005_AGE_SCALE
#define TUNE_005_AGE_SCALE 1.0
#endif
#ifndef TUNE_080_BARRIER
#define TUNE_080_BARRIER 0
#endif
#ifndef TUNE_090_BARRIER
#define TUNE_090_BARRIER 0
#endif
#ifndef TUNE_080_JOINT
#define TUNE_080_JOINT 0
#endif
#ifndef TUNE_090_JOINT
#define TUNE_090_JOINT 0
#endif
#ifndef TUNE_065_JOINT
#define TUNE_065_JOINT 1
#endif
#ifndef TUNE_067_JOINT
#define TUNE_067_JOINT 0
#endif
#ifndef TUNE_075_JOINT
#define TUNE_075_JOINT 1
#endif
#ifndef TUNE_015_JOINT
#define TUNE_015_JOINT 0
#endif
#ifndef TUNE_025_JOINT
#define TUNE_025_JOINT 1
#endif
#ifndef TUNE_030_JOINT
#define TUNE_030_JOINT 1
#endif
#ifndef TUNE_005_JOINT
#define TUNE_005_JOINT 0
#endif
#ifndef TUNE_045_JOINT
#define TUNE_045_JOINT 0
#endif
#ifndef TUNE_058_JOINT
#define TUNE_058_JOINT 0
#endif
#ifndef TUNE_HEAVY_JOINT_GAIN
#define TUNE_HEAVY_JOINT_GAIN 0.03
#endif
#ifndef TUNE_ZERO_HARD
#define TUNE_ZERO_HARD 1
#endif
#ifndef TUNE_045_PREFILL_SLICE
#define TUNE_045_PREFILL_SLICE 0.5
#endif
#ifndef TUNE_005_PREFILL_SLICE
#define TUNE_005_PREFILL_SLICE 0.5
#endif
#ifndef TUNE_058_PREFILL_SLICE
#define TUNE_058_PREFILL_SLICE 0.5
#endif
...
int main() {
    ...
    const bool bulk_decode_profile = (weight_is(0.80) && TUNE_080_BARRIER) ||
                                     (weight_is(0.90) && TUNE_090_BARRIER);
    
    
    
    const bool hard_zero_profile = w_tp <= 1e-12 && dist_base < 2.0 && TUNE_ZERO_HARD;
    const bool accurate_prefill_load = (w_tp <= 1e-12 && (TUNE_ZERO_ACCURATE_LOAD || hard_zero_profile)) ||
    ...
    if ((w_tp >= 0.985 && w_tp < 0.995 && TUNE_099_JOINT) ||
        ...
        (weight_is(0.05) && TUNE_005_JOINT) ||
        (weight_is(0.15) && TUNE_015_JOINT) ||
        (weight_is(0.25) && TUNE_025_JOINT) ||
        (weight_is(0.30) && TUNE_030_JOINT) ||
        (weight_is(0.45) && TUNE_045_JOINT) ||
        (weight_is(0.58) && TUNE_058_JOINT) ||
        (weight_is(0.65) && TUNE_065_JOINT) ||
        (weight_is(0.67) && TUNE_067_JOINT) ||
        (weight_is(0.75) && TUNE_075_JOINT) ||
        (weight_is(0.80) && TUNE_080_JOINT) ||
        (weight_is(0.90) && TUNE_090_JOINT)) {
        double base_joint_unit = steady_decode_unit;
        ...
        bool heavy_joint = (weight_is(1.00) && TUNE_100_JOINT) ||
                           (weight_is(0.05) && TUNE_005_JOINT) ||
                           (weight_is(0.15) && TUNE_015_JOINT) ||
                           (weight_is(0.25) && TUNE_025_JOINT) ||
                           (weight_is(0.30) && TUNE_030_JOINT) ||
                           ...
                           (weight_is(0.80) && TUNE_080_JOINT) ||
                           (weight_is(0.90) && TUNE_090_JOINT);
        if (!heavy_joint || joint_unit <= base_joint_unit * (1.0 - TUNE_HEAVY_JOINT_GAIN)) {
    ...
    auto cohort_target = [&]() -> int {
        ...
        double cohort_mult = weight_is(0.15) ? TUNE_015_COHORT_MULT :
                             weight_is(0.25) ? TUNE_025_COHORT_MULT :
                             weight_is(0.30) ? TUNE_030_COHORT_MULT :
                             weight_is(0.65) ? TUNE_065_COHORT_MULT :
                             weight_is(0.75) ? TUNE_075_COHORT_MULT :
                             weight_is(0.80) ? TUNE_080_COHORT_MULT :
    ...
    const double pre_age_coef =
        ...
        (w_tp <= 1e-12 ? (hard_zero_profile ? 0.0 : TUNE_ZERO_AGE_SCALE) :
    ...
    auto choose_piece_end = [&](int id, int k, double now) {
        ...
        double zero_slice = hard_zero_profile ? 1.0 : TUNE_ZERO_PREFILL_SLICE;
        if (w_tp <= 1e-12 && zero_slice > 0.0 &&
            pref_open >= 2 && num_layers > 1) {
            double full = dur(PPROC, req[id].lin);
            double per_layer = full / num_layers;
            double quantum = max(2.0 * S, zero_slice * SLO1);
            int span = max(1, (int)floor((quantum - S) / max(per_layer, 1e-18)));
            span = min(span, rem);
            if (span < rem) return ls + span;
        }
        if (weight_is(0.45) && TUNE_045_PREFILL_SLICE > 0.0 &&
            pref_open >= 2 && num_layers > 1) {
            double full = dur(PPROC, req[id].lin);
            double per_layer = full / num_layers;
            double quantum = max(4.0 * S, TUNE_045_PREFILL_SLICE * SLO1);
            if (S + per_layer * rem > 1.20 * quantum) {
                int span = max(1, (int)floor((quantum - S) / max(per_layer, 1e-18)));
                span = min(span, rem);
                if (span < rem) return ls + span;
            }
        }
        double profile_slice = weight_is(0.05) ? TUNE_005_PREFILL_SLICE :
                               weight_is(0.58) ? TUNE_058_PREFILL_SLICE : 0.0;
        if (profile_slice > 0.0 && pref_open >= 2 && num_layers > 1) {
            double full = dur(PPROC, req[id].lin);
            double per_layer = full / num_layers;
            double quantum = max(4.0 * S, profile_slice * SLO1);
            if (S + per_layer * rem > 1.20 * quantum) {
                int span = max(1, (int)floor((quantum - S) / max(per_layer, 1e-18)));
                span = min(span, rem);
                if (span < rem) return ls + span;
    ...
    while (cin >> head) {
        ...
        if (!busyE) {
            ...
            if (!qDPRE.empty()) {
                ...
                if (bulk_decode_profile &&
                    (pref_open > 0 || avail < total_decode_active())) avail = 0;
                if (avail > 0) {
            ...
            if (!qDPOST.empty()) {
                ...
                if (bulk_decode_profile &&
                    n + hot_dpre_ready + cold_dpre_ready < total_decode_active()) n = 0;
                if (n > 0) {
            ...
            if (pick >= 0) {
                ...
                if (c.type == PPRE) {
                    ...
                    if (bulk_decode_profile &&
                        (pref_open > 0 || n < total_decode_active())) n = 0;
