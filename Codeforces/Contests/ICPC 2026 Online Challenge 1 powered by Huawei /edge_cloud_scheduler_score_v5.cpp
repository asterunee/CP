...
int main() {
    ...
    auto batch_for = [&](int t, int n) -> int {
        n = max(1, min(MAXR, n));
        if (sched_wtp >= 0.72) {
            // For a throughput-dominated test, do not keep the original static
            // score mix after the SLO component has already saturated.
            int b = bestMakeB[t][n];
    ...
    const double sjf_mix = loose_slo ? 0.0 : clamp((w_c - 0.50) / 0.50, 0.0, 1.0);
    ...
    while (cin >> head) {
        ...
        if (!busyE) {
            ...
            for (int z = 0; z < (int)cs.size(); ++z) {
                auto c = cs[z];
                if (c.type == DPOST && pendingDownDecReq > 0 && c.eff > now) {
                    int n = min(MAXR, (int)qDPOST.size());
                    bool can_wait = false;
                    double dt = 0.0;
