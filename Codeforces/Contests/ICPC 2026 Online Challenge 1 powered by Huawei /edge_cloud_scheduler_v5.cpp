...
int main() {
    ...
    const bool high_batch_profile = weight_is(0.80);
    const bool throughput_heavy_profile = weight_is(0.80) || weight_is(0.90);
    const bool balanced_batch_profile = weight_is(0.65);
    ...
    const bool accurate_prefill_load = (w_tp > 1e-12 && w_tp < 0.20) ||
                                       (w_tp >= 0.45 && !weight_is(0.50));
    ...
    bool latency_serial = false;
    bool low_weight_serial = false;
    bool saturated_latency_narrow = false;
    ...
    auto choose_piece_end = [&](int id, int k, double now) {
        ...
        if (w_tp <= 0.15 + 1e-12 && !loose_slo && pref_open >= 2 && K == 8) {
    ...
    while (cin >> head) {
        ...
        auto decode_window_target = [&]() -> int {
            int t = cohort_target();
            if (w_tp <= 1e-12 && latency_serial) t = 1;
            else if (weight_is(0.15) && low_weight_serial) t = 1;
            else if (w_tp <= 1e-12 && finished_request_count >= 4 && estimated_lout() > 4.0) t = 1;
            else if (weight_is(0.67) && saturated_latency_narrow) t = max(K, (t + 1) / 2);
        ...
        if (w_tp <= 1e-12 && tpot_cnt + gap_open >= 4) {
            latency_serial = focus.dec_excess > 0.02 && focus.dec_share >= TUNE_LAT_SHARE;
        }
        if (weight_is(0.15) && tpot_cnt + gap_open >= 8) {
            low_weight_serial = focus.dec_excess > 0.02 && focus.dec_share >= TUNE_LOW_SHARE;
        }
        ...
        if (produced_tokens >= 4 && first_arrival < 1e290 && now > first_arrival + 1e-12) {
            ...
            if (weight_is(0.67) && finished_request_count >= 8) {
                saturated_latency_narrow = ntp > 0.98 && focus.dec_excess > 0.02 &&
                                           focus.dec_share >= 0.55;
            }
            ...
            if (weight_is(0.30) && produced_tokens >= 16 && nc > 0.93 && ntp < 0.75) {
                aw_c *= 0.10;
            }
        ...
        for (int k = 0; k < K; ++k) {
            ...
            if (one_shot_mode() && hasP) chooseD = false;
