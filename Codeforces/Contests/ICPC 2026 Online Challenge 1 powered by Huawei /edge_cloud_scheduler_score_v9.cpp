...
int main() {
    ...
    // Jointly optimize steady-state decode cohort size and the number of
    // remotes used by that cohort.  Using all K remotes is not always optimal:
    // every represented remote adds one UP and one DOWN latency startup.
    int steady_decode_batch = 1;
    int steady_decode_clouds = 1;
    ...
    for (int H = 1; H <= K; ++H) {
        for (int B = 1; B <= MAXR; ++B) {
            ...
            // Exact aggregate cloud work for the balanced split.
            double cloud_work =
                (m - r) * dcost[DPROC][q] +
                (r ? r * dcost[DPROC][q + 1] : 0.0);
            double cloud_unit = cloud_work / (double(B) * m);

            // Also account for the slowest individual remote.  This prevents a
            // misleading aggregate optimum when DPROC has a strongly non-linear
            // batch curve.
            int mx = q + (r != 0);
            double cloud_span_unit = dcost[DPROC][max(1, mx)] / max(1, mx);

            double u = max({edge_unit, link_unit, cloud_unit, cloud_span_unit / m});

            if (u < steady_decode_unit - 1e-12 ||
                (abs(u - steady_decode_unit) <= 1e-12 &&
                 (B < steady_decode_batch ||
                  (B == steady_decode_batch && H < steady_decode_clouds)))) {
                steady_decode_unit = u;
                steady_decode_batch = B;
                steady_decode_clouds = H;
            }
        }
    }
    ...
    auto cohort_target = [&]() -> int {
        ...
        // Short-output tests are dominated by admission/prefill completion, not by
        // steady-state decode.  Waiting for hundreds of requests here is disastrous.
        if (finished_request_count >= 4) {
            if (lo <= 1.20) cap = min(cap, max(2, 2 * K));
            else if (lo <= 2.25) cap = min(cap, max(4, 4 * K));
            else if (lo <= 4.0) cap = min(cap, max(8, 8 * K));
        }
    ...
    // The first produced token has no TPOT penalty at all.  Treating the
    // first decode exactly like later decode iterations wastes latency budget:
    // it can delay P POSTs (hurting TDR) and can delay requests whose next-token
    // gaps actually count.  A pseudo-deadline separates the two classes.
    ...
    auto desired_cloud_count = [&]() -> int {
        // Latency-heavy workloads use every cloud.  For throughput-heavy or
        // huge-SLO workloads use the analytically best number of remotes, but
        // do not collapse the system before we have any evidence that outputs
        // are long-lived unless throughput weight is overwhelming.
        if (loose_slo && w_tp > 1e-12) return steady_decode_clouds;
        if (w_tp >= 0.90) return steady_decode_clouds;
        if (w_tp >= 0.80 && finished_request_count >= 4 && estimated_lout() > 2.25)
            return steady_decode_clouds;
        return K;
    };
    ...
    auto choose_remote = [&](double now) {
        ...
        int H = max(1, min(K, desired_cloud_count()));
        ...
        for (int k = 0; k < H; ++k) {
            ...
            if (sched_wtp >= 0.72) {
                ...
                // D PRE enqueues UP transfers in increasing remote index.  Give
                // earlier remotes a tiny bias toward more work so their compute
                // can overlap transfers to later remotes.
                score += k * 0.015 * max(1.0, dcost[DPROC][1]);
            } else {
    ...
    while (cin >> head) {
        ...
        if (produced_tokens >= (w_tp >= 0.70 ? 96 : 32) && first_arrival < 1e290 && now > first_arrival + 1e-12) {
            ...
            if (ntp > 0.96) aw_tp *= max(0.06, (1.0 - ntp) / 0.04);
            if (nc > 0.97) aw_c *= max(0.06, (1.0 - nc) / 0.03);
        }
        if (aw_tp + aw_c > 1e-15) {
        ...
        for (int k = 0; k < K; ++k) {
            ...
            if (chooseD && sched_wtp >= 0.12) {
                ...
                bool first_token_wait_forbidden =
                    top_id_for_wait >= 0 && req[top_id_for_wait].tokens == 0 && sched_wc >= 0.45;
        ...
        if (!busyE) {
            ...
            if (!qDPRE.empty()) {
                ...
                cs.push_back({norm_key(eff, now, SLO2, cont_bonus) - 1.20 * dec_bias,
                              eff, dcost[DPRE][b], DPRE, b});
            }
            if (!qDPOST.empty()) {
            ...
            if (forced_pick < 0 && sched_wtp >= 0.58 && !qPPOST.empty() && !qDPRE.empty()) {
                int ct = cohort_target();
            ...
            for (int z = 0; pick < 0 && z < (int)cs.size(); ++z) {
                ...
                if (c.type == DPOST && sched_wtp >= 0.12 && pendingDownDecReq > 0 && c.eff > now) {
                    ...
                    if (top_post_id >= 0 && req[top_post_id].tokens == 0 && sched_wc >= 0.45) {
                        pick = z;
                        break;
                    }
                    int n = min(MAXR, (int)qDPOST.size());
