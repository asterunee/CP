...
int main() {
    ...
    // These score profiles are one-token workloads in the preliminary suite.
    // Finish admission first, then drain the token-free cold decode pool in
    // throughput-optimal batches.
    const bool one_shot_profile = weight_is(0.05) || weight_is(0.45) || weight_is(0.58);
    const bool high_batch_profile = weight_is(0.80) || weight_is(0.90);
    ...
    while (cin >> head) {
        ...
        for (int id : fins) {
            if (req[id].st != FINISHED) {
                ...
                learned_one_shot = finished_request_count >= 8 &&
        ...
        auto decode_window_target = [&]() -> int {
            int t = cohort_target();
            if (w_tp <= 1e-12 && finished_request_count >= 4 && estimated_lout() > 4.0) t = 1;
