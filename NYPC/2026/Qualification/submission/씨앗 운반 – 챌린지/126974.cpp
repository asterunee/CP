...
static void run_tail_general_search(const vector<ll>& A, const vector<ll>& B, int T, int M,
                                    uint32_t base_seed, chrono::steady_clock::time_point begin,
                                    chrono::steady_clock::time_point end,
...
int main() {
        ...
        for (int ai = 0; ai < int(k3.size()) && ai < 12 && !construction_deadline.expired(); ++ai) {
            for (int d = 3; d <= 9 && !construction_deadline.expired(); ++d) {
                ...
                for (int v = 0; v < 40 && !construction_deadline.expired(); ++v) {
                    Board cand;
                    if (!build_split_board(A, k3[ai].dst, M, rr, 3,
                            base_seed ^ uint32_t(0x165667B1u + ai * 10007u + rr * 257u + v),
    ...
    run_tail_general_search(A, B, T, M, base_seed, begin, begin + chrono::milliseconds(1940), best_result, best_board);
...
static void free_init(FreeState& s, const vector<ll>& A, const vector<ll>& B,
    ...
    for (int item : order) {
        ...
        ll bs = (1LL << 62);
        for (int d = 0; d < s.C; ++d) {
            ll z = llabs(s.sum[d] + s.value[item] - B[d]) - llabs(s.sum[d] - B[d]);
            z += int(rng.next() & 255u);
            if (z < bs) bs = z, bd = d;
        }
        s.dest[item] = bd;
...
static inline void free_move(FreeState& s, int item, int nd, const vector<ll>& B) {
    int od = s.dest[item];
    if (od == nd) return;
...
static int estimate_queue_delay_free(const vector<ll>& A, const vector<int>& dest, int target) {
    ...
    for (int id = 0; id < 4 * C; ++id) if (dest[id] == target) {
        ...
        if (cnt <= 0) continue;
        // Time is shifted by M. Each stream emits once per four seconds.
    ...
    // backlog16 stores sixteen times the fluid backlog. Service rate is 1 seed/s.
    ...
    // prev is in quarter-second*4 coordinates; backlog16 / 16 is seeds.
    // Drain at one per second after the final arrival.
    return max(0, (prev + 3) / 4 + int((backlog16 + 15) / 16));
}

static ll free_balanced_objective(const vector<ll>& A, const FreeState& s) {
    int d = 0;
    for (int j = 0; j < s.C; ++j) d = max(d, estimate_queue_delay_free(A, s.dest, j));
...
static vector<FreeQueueAssignment> make_free_queue_assignments(
    ...
    while (chrono::steady_clock::now() < local_end) {
        ...
        for (int it = 0; it < ITER; ++it) {
            ...
            double temp = 150000.0 * pow(0.0005, double(it) / ITER);
            bool accept = after <= before;
            if (!accept) {
                double prob = exp(double(before - after) / max(1.0, temp));
                accept = prob > (double(rng.next()) + .5) / 4294967296.0;
            }
            if (accept) cur = after;
            else free_move(s, item, od, B);

            if ((it & 31) == 31) {
                // Exact best single relocation under the current state.
                ll bd = 0;
                int bi = -1, bt = -1;
                for (int x = 0; x < s.N; ++x) {
                    int from = s.dest[x];
...
static bool add_gadget_cell(vector<pair<int,string>>& cells, int id, const string& tok) {
    for (auto& z : cells) if (z.first == id) {
        return z.second == tok && vertical_token(tok);
    }
    cells.push_back({id, tok});
    return true;
}
...
static bool build_csp_queue_board(const vector<ll>& A, const vector<int>& dst,
    ...
    for (int src = 0; src < C; ++src) {
        ...
        if (centers.empty()) return false;
        const int ATT = (C <= 7 ? 5000 : 3500);
        for (int it = 0; it < ATT && chrono::steady_clock::now() < end; ++it) {
            ...
            bool ok = true;
            auto put = [&](int r, int c, const string& tok) {
                if (r < 0 || r >= R || c < 0 || c >= C) return false;
                return add_gadget_cell(cells, r * C + c, tok);
            };
            ...
            if (mode == 0) {
                ...
                int entry;
                if (mode == 1) entry = 1 + rng.mod(max(1, root - 1));
                else entry = max(1, root / 2);
            ...
            bool dup = false;
            sort(cells.begin(), cells.end(), [](auto& x, auto& y){return x.first<y.first;});
            for (auto& old : cand[src]) if (old.cell == cells) { dup=true; break; }
            if (!dup) cand[src].push_back({move(cells),score});
            if (cand[src].size() >= 180) break;
        }
        if (cand[src].empty()) return false;
        sort(cand[src].begin(), cand[src].end(), [](const auto& x,const auto& y){return x.score<y.score;});
    }
    ...
    function<bool(int)> dfs = [&](int dep) -> bool {
        ...
        int src = -1, bestcnt = INT_MAX;
        for (int s = 0; s < C; ++s) if (!used[s]) {
            int cnt = 0;
            for (const auto& q : cand[s]) {
                bool ok = true;
                for (auto& z : q.cell) if (refs[z.first] && board[z.first] != z.second) { ok=false; break; }
                if (ok && ++cnt >= bestcnt) break;
            }
            if (cnt < bestcnt) bestcnt = cnt, src = s;
            if (cnt == 0) break;
        }
        if (src < 0 || bestcnt == 0) return false;
        used[src] = 1;
        int start = cand[src].empty() ? 0 : rng.mod(min<int>(7,cand[src].size()));
        int lim = min<int>(cand[src].size(), C <= 7 ? 180 : 120);
...
static void run_tail_general_search(const vector<ll>& A, const vector<ll>& B, int T, int M,
    ...
    if (C > 7 || best_result.cost <= 10000 || now + chrono::milliseconds(90) >= end) return;

    auto assign_end = min(end, now + chrono::milliseconds(260));
    vector<FreeQueueAssignment> fq = make_free_queue_assignments(A, B, assign_end,
                                                                  base_seed ^ 0x6A09E667u);
    ...
    sort(pool.begin(), pool.end(), [](const TailCandidate& x, const TailCandidate& y) {
        return x.key < y.key;
    });
    int checked = 0;
    for (int i = 0; i < int(pool.size()) && chrono::steady_clock::now() + chrono::milliseconds(15) < end; ++i) {
        SimResult r = simulate_board(A, B, T, M, pool[i].board);
        if (better_result(r, pool[i].board, best_result, best_board)) {
            best_result = r;
            best_board = move(pool[i].board);
