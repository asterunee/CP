...
int main() {
    ...
    const Deadline deadline{begin + chrono::milliseconds(950)};
    ...
    for (int K : {4, 3, 2}) {
        ...
        if (!exact_simulation && C == 5 && K == 4 && !p.candidates.empty() &&
            p.candidates[0].err >= 40000 && !deadline.expired()) {
            vector<AssignmentK> exact = exact_k4_c5(A, B, deadline);
            for (AssignmentK& x : exact) {
                bool duplicate = false;
                for (const AssignmentK& y : p.candidates) if (x.dst == y.dst) { duplicate = true; break; }
                if (!duplicate) p.candidates.push_back(move(x));
            }
            sort(p.candidates.begin(), p.candidates.end(), [](const AssignmentK& a, const AssignmentK& b) {
                return a.err < b.err;
            });
