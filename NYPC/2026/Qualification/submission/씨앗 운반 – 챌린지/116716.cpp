...
static vector<AssignmentK> make_assignments(const vector<ll>& A, const vector<ll>& B,
    ...
    const int local_iterations = 70000;
    ...
    for (int rep = 0; rep < repetitions; ++rep) {
        ...
        for (int it = 0; it < local_iterations; ++it) {
            if ((it & 8191) == 0 && deadline.expired()) break;
            const int p = rng.mod(N);
            const int q = rng.mod(N);
            if (p == q || p / K == q / K) continue;

            const int x = slot[p];
            const int y = slot[q];
            const int u = p / K;
            const int v = q / K;
            const ll old_part = llabs(sum[u] - B[u]) + llabs(sum[v] - B[v]);

            sum[u] += value[y] - value[x];
            sum[v] += value[x] - value[y];
            const ll new_part = llabs(sum[u] - B[u]) + llabs(sum[v] - B[v]);

            bool accept = new_part <= old_part;
            if (!accept && (it % 4000) < 40) accept = true;

            if (accept) {
                swap(slot[p], slot[q]);
                current += new_part - old_part;
...
struct RouteEdit {
    uint8_t r = 0, c = 0;
    uint8_t kind = 0; // 0: hamster, 1: squirrel
    uint8_t tr = 0, tc = 0;
    uint8_t k = 0;
    uint8_t dir[4] = {};
};
...
static bool build_split_board(const vector<ll>& A, const vector<int>& dst,
    ...
    for (int source : order) {
        ...
        avail[avail_count++] = 0;
        avail[avail_count++] = 1;
        if (source > 0) avail[avail_count++] = 2;
        if (source + 1 < n) avail[avail_count++] = 3;
        if (avail_count < K) return false;
        ...
        for (int y = 2; y <= R - 1; ++y) {
            auto evaluate = [&]() {
                ...
                for (int k = 0; k < K; ++k) {
                    ...
                    const int base = int(((M - A[source] + 1) + k + 2 + (lc != b)) % K);
                    int best_delay = -1;
                    int best_residue = -1;
                    int best_rows[4] = {};
                ...
                add_squirrel_edit(p, y - 1, source, perm, K);
                add_hamster_edit(p, 0, source, y - 1, source);
                insert_plan(top, top_count, 6, p);
            };
        ...
        const int pick_limit = min(4, top_count);
...
int main() {
    ...
    const Deadline deadline{begin + chrono::milliseconds(700)};
    ...
    for (int K : {2, 3, 4}) {
        ...
        if (K == 4) {
            bool possible = C >= 3;
            for (int i = 0; i < C; ++i) {
                if (A[i] > 0 && (i == 0 || i + 1 == C)) possible = false;
            }
    ...
    for (const Phase& phase : phases) {
        ...
        const int keep = (K == 2 ? 6 : K == 3 ? 7 : 8);
        const int variants = (K == 2 ? 38 : K == 3 ? 54 : 64);
        const int extra_rows = (K == 2 ? 3 : K == 3 ? 4 : 5);

        for (int ai = 0; ai < int(phase.candidates.size()) && ai < keep; ++ai) {
            if (deadline.expired()) break;
            if (1 + phase.candidates[ai].err >= best_result.cost) break;

            for (int rr = C; rr <= min(C + extra_rows, C + 20); ++rr) {
                if (deadline.expired()) break;
