...
static int mixed_value(const vector<ll>& A, int id) {
    const int s = id / 5, k = id % 5;
    if (k < 3) return int((A[s] + 2 - k) / 4);
    const int residue = k == 3 ? 0 : 4;
    return int((A[s] + 7 - residue) / 8);
}

static MixedAssignment eval_mixed(const vector<ll>& A, const vector<ll>& B,
                                  const vector<int>& dst) {
    const int C = int(A.size());
    ll sum[MAX_C] = {};
    for (int id = 0; id < 5 * C; ++id) sum[dst[id]] += mixed_value(A, id);
    MixedAssignment q;
    q.dst = dst;
    q.err = 0;
    for (int i = 0; i < C; ++i) q.err += llabs(sum[i] - B[i]);
    return q;
}

struct MixedState {
    int C = 0;
    vector<int> value, coarse, fine, dest;
    vector<ll> sum;
    ll err = 0;
};

static void mixed_recompute(MixedState& s, const vector<ll>& B) {
    fill(s.sum.begin(), s.sum.end(), 0);
    for (int id = 0; id < 5 * s.C; ++id) s.sum[s.dest[id]] += s.value[id];
    s.err = 0;
    for (int g = 0; g < s.C; ++g) s.err += llabs(s.sum[g] - B[g]);
}

static void mixed_init(MixedState& s, const vector<ll>& A, const vector<ll>& B,
                       FastRNG& rng) {
    s.C = int(A.size());
    s.value.resize(5 * s.C);
    s.dest.assign(5 * s.C, -1);
    s.coarse.clear();
    s.fine.clear();
    for (int id = 0; id < 5 * s.C; ++id) {
        s.value[id] = mixed_value(A, id);
        (id % 5 < 3 ? s.coarse : s.fine).push_back(id);
    }
    shuffle_ints(s.coarse, rng);
    shuffle_ints(s.fine, rng);
    stable_sort(s.coarse.begin(), s.coarse.end(), [&](int x, int y) {
        return s.value[x] > s.value[y];
    });
    stable_sort(s.fine.begin(), s.fine.end(), [&](int x, int y) {
        return s.value[x] > s.value[y];
    });
    s.sum.assign(s.C, 0);
    int cnt[MAX_C] = {};
    ll coarse_total = 0;
    for (int id : s.coarse) coarse_total += s.value[id];
    vector<int> placed(3 * s.C);
    for (int id : s.coarse) {
        int bg = -1;
        ll bs = (1LL << 62);
        for (int g = 0; g < s.C; ++g) if (cnt[g] < 3) {
            const ll target = B[g] * coarse_total / 1000000;
            ll score = llabs(s.sum[g] + s.value[id] - target)
                     - llabs(s.sum[g] - target) + rng.mod(257);
            if (score < bs) bs = score, bg = g;
        }
        placed[3 * bg + cnt[bg]++] = id;
        s.dest[id] = bg;
        s.sum[bg] += s.value[id];
    }
    s.coarse.swap(placed);
    fill(cnt, cnt + s.C, 0);
    placed.assign(2 * s.C, 0);
    for (int id : s.fine) {
        int bg = -1;
        ll bs = (1LL << 62);
        for (int g = 0; g < s.C; ++g) if (cnt[g] < 2) {
            ll score = llabs(s.sum[g] + s.value[id] - B[g])
                     - llabs(s.sum[g] - B[g]) + rng.mod(129);
            if (score < bs) bs = score, bg = g;
        }
        placed[2 * bg + cnt[bg]++] = id;
        s.dest[id] = bg;
        s.sum[bg] += s.value[id];
    }
    s.fine.swap(placed);
...
static void mixed_init_k4(MixedState& s, const vector<ll>& A,
                          const vector<ll>& B, const AssignmentK& base) {
    s.C = int(A.size());
    s.value.resize(5 * s.C);
    s.dest.assign(5 * s.C, -1);
    s.coarse.clear();
    ...
    for (int src = 0; src < s.C; ++src) {
        for (int k = 0; k < 5; ++k) {
            const int id = 5 * src + k;
            s.value[id] = mixed_value(A, id);
            (k < 3 ? s.coarse : s.fine).push_back(id);
        }
        s.dest[5 * src + 0] = base.dst[4 * src + 1];
        s.dest[5 * src + 1] = base.dst[4 * src + 2];
        s.dest[5 * src + 2] = base.dst[4 * src + 3];
        s.dest[5 * src + 3] = s.dest[5 * src + 4] = base.dst[4 * src];
    }
    s.sum.assign(s.C, 0);
    mixed_recompute(s, B);
}

static void mixed_swap(MixedState& s, vector<int>& slot, int arity,
                       int p, int q, const vector<ll>& B) {
    if (p == q || p / arity == q / arity) return;
    const int a = p / arity, b = q / arity;
    const int x = slot[p], y = slot[q];
    const ll old = llabs(s.sum[a] - B[a]) + llabs(s.sum[b] - B[b]);
    s.sum[a] += s.value[y] - s.value[x];
    s.sum[b] += s.value[x] - s.value[y];
    swap(slot[p], slot[q]);
    s.dest[x] = b;
    s.dest[y] = a;
    s.err += llabs(s.sum[a] - B[a]) + llabs(s.sum[b] - B[b]) - old;
}

static bool mixed_optimize_pair(MixedState& s, int a, int b,
                                const vector<ll>& B) {
    int ci[6], fi[4];
    for (int k = 0; k < 3; ++k) {
        ci[k] = s.coarse[3 * a + k];
        ci[3 + k] = s.coarse[3 * b + k];
    }
    for (int k = 0; k < 2; ++k) {
        fi[k] = s.fine[2 * a + k];
        fi[2 + k] = s.fine[2 * b + k];
    }
    const ll total = s.sum[a] + s.sum[b];
    ll best = llabs(s.sum[a] - B[a]) + llabs(s.sum[b] - B[b]);
    int bcm = -1, bfm = -1;
    for (int cm = 0; cm < 64; ++cm) if (__builtin_popcount(unsigned(cm)) == 3) {
        ll sc = 0;
        for (int z = 0; z < 6; ++z) if (cm >> z & 1) sc += s.value[ci[z]];
        for (int fm = 0; fm < 16; ++fm) if (__builtin_popcount(unsigned(fm)) == 2) {
            ll sa = sc;
            for (int z = 0; z < 4; ++z) if (fm >> z & 1) sa += s.value[fi[z]];
            const ll e = llabs(sa - B[a]) + llabs(total - sa - B[b]);
            if (e < best) best = e, bcm = cm, bfm = fm;
        }
    }
    ...
    int ca = 0, cb = 0;
    for (int z = 0; z < 6; ++z) {
        const int g = (bcm >> z & 1) ? a : b;
        s.coarse[3 * g + (g == a ? ca++ : cb++)] = ci[z];
        s.dest[ci[z]] = g;
        s.sum[g] += s.value[ci[z]];
    }
    ca = cb = 0;
    for (int z = 0; z < 4; ++z) {
        const int g = (bfm >> z & 1) ? a : b;
        s.fine[2 * g + (g == a ? ca++ : cb++)] = fi[z];
        s.dest[fi[z]] = g;
        s.sum[g] += s.value[fi[z]];
    }
    s.err += llabs(s.sum[a] - B[a]) + llabs(s.sum[b] - B[b]);
    return true;
}

static void insert_mixed(vector<MixedAssignment>& out, const MixedState& s,
                         int limit) {
    for (const auto& q : out) if (q.dst == s.dest) return;
    out.push_back({s.dest, s.err});
    sort(out.begin(), out.end(), [](const MixedAssignment& x, const MixedAssignment& y) {
        return x.err < y.err;
    });
    if (int(out.size()) > limit) out.resize(limit);
}

static inline int mixed_weight(int id) {
    return id % 5 < 3 ? 2 : 1;
}

static void mixed_swap_ids(MixedState& s, int x, int y,
                           const vector<ll>& B) {
    const int a = s.dest[x], b = s.dest[y];
    if (a == b || mixed_weight(x) != mixed_weight(y)) return;
    const ll old = llabs(s.sum[a] - B[a]) + llabs(s.sum[b] - B[b]);
    s.sum[a] += s.value[y] - s.value[x];
    s.sum[b] += s.value[x] - s.value[y];
    s.dest[x] = b;
...
static bool mixed_exchange(MixedState& s, int x, int y, int z,
                           const vector<ll>& B) {
    const int a = s.dest[x], b = s.dest[y];
    if (mixed_weight(x) != 2 || mixed_weight(y) != 1 || mixed_weight(z) != 1 ||
        y == z || a == b || s.dest[z] != b) return false;
    const ll old = llabs(s.sum[a] - B[a]) + llabs(s.sum[b] - B[b]);
    s.sum[a] += s.value[y] + s.value[z] - s.value[x];
    s.sum[b] += s.value[x] - s.value[y] - s.value[z];
    s.dest[x] = b;
    s.dest[y] = s.dest[z] = a;
    s.err += llabs(s.sum[a] - B[a]) + llabs(s.sum[b] - B[b]) - old;
    return true;
...
static bool mixed_pair_flexible(MixedState& s, int a, int b,
                                const vector<ll>& B) {
    vector<int> item;
    for (int id = 0; id < 5 * s.C; ++id)
        if (s.dest[id] == a || s.dest[id] == b) item.push_back(id);
    ...
    for (int mask = 0; mask < lim; ++mask) {
        int w = 0;
        ll sa = 0;
        for (int z = 0; z < n; ++z) if (mask >> z & 1) {
            w += mixed_weight(item[z]);
            if (w > 8) break;
            sa += s.value[item[z]];
        }
        if (w != 8) continue;
        const ll e = llabs(sa - B[a]) + llabs(total - sa - B[b]);
        if (e < best) best = e, best_mask = mask;
    }
    if (best_mask < 0) return false;
    s.err -= llabs(s.sum[a] - B[a]) + llabs(s.sum[b] - B[b]);
    s.sum[a] = s.sum[b] = 0;
    for (int z = 0; z < n; ++z) {
        const int g = best_mask >> z & 1 ? a : b;
        s.dest[item[z]] = g;
        s.sum[g] += s.value[item[z]];
    }
    s.err += llabs(s.sum[a] - B[a]) + llabs(s.sum[b] - B[b]);
    return true;
}

static vector<MixedAssignment> make_mixed_assignments(
        const vector<ll>& A, const vector<ll>& B,
        const vector<AssignmentK>& seeds,
        chrono::steady_clock::time_point end, uint32_t seed) {
    FastRNG rng(seed ^ 0xD6E8FEB8u);
    vector<MixedAssignment> out;
    out.reserve(32);
    int restart = 0;
    while (chrono::steady_clock::now() < end) {
        MixedState s;
        if (!seeds.empty() && (restart & 1) == 0)
            mixed_init_k4(s, A, B, seeds[(restart >> 1) % min<int>(8, seeds.size())]);
        else
            mixed_init(s, A, B, rng);
        ++restart;
        insert_mixed(out, s, 32);
        ll local = (1LL << 60);
        const int iterations = 140000;
        for (int it = 0; it < iterations; ++it) {
            if ((it & 127) == 0 && chrono::steady_clock::now() >= end) break;
            const ll before = s.err;
            int mode = rng.mod(5);
            int x = -1, y = -1, z = -1;
            if (mode < 4) {
                vector<int>& item = mode & 1 ? s.coarse : s.fine;
                x = item[rng.mod(int(item.size()))];
                y = item[rng.mod(int(item.size()))];
                if (s.dest[x] == s.dest[y]) continue;
                mixed_swap_ids(s, x, y, B);
            } else {
                x = s.coarse[rng.mod(int(s.coarse.size()))];
                const int a = s.dest[x];
                y = s.fine[rng.mod(int(s.fine.size()))];
                const int b = s.dest[y];
                if (a == b) continue;
                vector<int> choice;
                for (int id : s.fine) if (id != y && s.dest[id] == b) choice.push_back(id);
                if (choice.empty()) continue;
                z = choice[rng.mod(int(choice.size()))];
                if (!mixed_exchange(s, x, y, z, B)) continue;
            }
            const ll after = s.err;
            const double temp = 100000.0 * pow(0.0003, double(it) / iterations);
            bool accept = after <= before;
            if (!accept) accept = exp(double(before - after) / max(1.0, temp))
                                > (double(rng.next()) + .5) / 4294967296.0;
            if (!accept) {
                if (mode < 4) mixed_swap_ids(s, x, y, B);
                else mixed_exchange(s, x, y, z, B);
            }
            if ((it & 1023) == 1023) {
                bool changed = false;
                for (int rep = 0; rep < s.C; ++rep) {
                    int a = rng.mod(s.C), b = rng.mod(s.C);
                    if (a != b) changed |= mixed_pair_flexible(s, a, b, B);
                }
                if (!changed && it > 4096) break;
            }
            if (s.err < local) {
                local = s.err;
                insert_mixed(out, s, 32);
...
static bool place_mixed_skeleton(const vector<ll>& A, int M,
                                 const vector<int>& dst, int R, uint32_t seed,
                                 const Deadline& deadline, PhaseState& out) {
    const int C = int(A.size());
    FastRNG rng(seed ^ 0xA5A3564Du);
    static const int dr[4] = {-1, 1, 0, 0};
    static const int dc[4] = {0, 0, -1, 1};
    static const char ch[4] = {'U', 'D', 'L', 'R'};
    vector<int> order(C);
    iota(order.begin(), order.end(), 0);
    for (int restart = 0; restart < 500 && !deadline.expired(); ++restart) {
        PhaseState s{C, R, vector<uint16_t>(R),
                     vector<vector<string>>(R, vector<string>(C, "X")), {},
                     vector<int>(R * C, -1)};
        shuffle_ints(order, rng);
        bool all = true;
        for (int src : order) {
            if (A[src] == 0) continue;
            bool placed = false;
            for (int at = 0; at < 1200 && !deadline.expired() && !placed; ++at) {
                PhaseState q = s;
                const int rr = 2 + rng.mod(max(1, R - 3));
                const int cc = 1 + rng.mod(C - 2);
                if (cc != src && abs(cc - src) < 2) continue;
                auto take = [&](int r, int c) {
                    if (!phase_free(q, r, c)) return false;
                    phase_take(q, r, c);
                    return true;
                };
                if (!take(0, src) || !take(rr, cc)) continue;
                const int root_delay = cc == src ? 1 : 2;
                q.g[0][src] = jump_token(rr, 'D');
                if (cc != src) {
                    if (!take(rr, src)) continue;
                    q.g[rr][src] = jump_token(abs(cc - src), cc > src ? 'R' : 'L');
                }
                int root_dir[4] = {0, 1, 2, 3};
                for (int z = 3; z > 0; --z) swap(root_dir[z], root_dir[rng.mod(z + 1)]);
                const int refined = root_dir[0];
                int child_dir[3], nc = 0;
                for (int d = 0; d < 4; ++d) if (d != (refined ^ 1)) child_dir[nc++] = d;
                for (int z = 2; z > 0; --z) swap(child_dir[z], child_dir[rng.mod(z + 1)]);
                const int cr = rr + dr[refined], cc2 = cc + dc[refined];
                bool ok = take(cr, cc2);
                string rt;
                for (int z = 0; z < 4; ++z) rt.push_back(ch[root_dir[z]]);
                q.g[rr][cc] = rt;
                for (int z = 1; z < 4 && ok; ++z) {
                    const int tr = rr + dr[root_dir[z]], tc = cc + dc[root_dir[z]];
                    if (!take(tr, tc)) { ok = false; break; }
                    const int item = z - 1;
                    if (mixed_value(A, 5 * src + item) > 0)
                        q.terms.push_back({tr, tc, dst[5 * src + item], 4,
                            int((ll(M) - A[src] + 1 + z + root_delay) & 15)});
                }
                string ct;
                for (int z = 0; z < 2 && ok; ++z) {
                    ct.push_back(ch[child_dir[z]]);
                    const int tr = cr + dr[child_dir[z]], tc = cc2 + dc[child_dir[z]];
                    if (tr < 1 || tr >= R || tc < 0 || tc >= C || !take(tr, tc)) {
                        ok = false;
                        break;
                    }
                    const int item = 3 + z;
                    if (mixed_value(A, 5 * src + item) > 0)
                        q.terms.push_back({tr, tc, dst[5 * src + item], 8,
                            int((ll(M) - A[src] + 1 + 4 * z + root_delay + 1) & 15)});
                }
                if (!ok) continue;
                q.g[cr][cc2] = ct;
                s = move(q);
...
static bool build_mixed_board(const vector<ll>& A, int M,
                              const vector<int>& dst, int R, uint32_t seed,
                              const Deadline& deadline, Board& out) {
    PhaseState s;
    if (!place_mixed_skeleton(A, M, dst, R, seed, deadline, s)) return false;
    if (!route_phase_terms(s, seed ^ 0x3C6EF372u, deadline)) return false;
    out.R = R;
    out.g = move(s.g);
    return true;
}

struct MixedFreeAssignment {
    vector<int> dst;
    ll predicted = (1LL << 60);
    ll err = (1LL << 60);
};

struct MixedFreeState {
    int C = 0, N = 0;
    vector<int> value, dest;
    vector<ll> sum;
    ll err = 0;
};

static int mixed_free_delay(const vector<ll>& A, const vector<int>& dest,
                            int target) {
    struct Event { int t, d; };
    Event ev[120];
    int ec = 0;
    for (int id = 0; id < int(dest.size()); ++id) if (dest[id] == target) {
        const int src = id / 5, k = id % 5;
        const int period = k < 3 ? 4 : 8;
        const int residue = k < 3 ? k + 1 : (k == 3 ? 0 : 4);
        const int count = mixed_value(A, id);
        if (count <= 0) continue;
        const int first = 1 - int(A[src]) + residue;
        const int last = first + period * (count - 1);
        const int rate8 = 8 / period;
        ev[ec++] = {first, rate8};
        ev[ec++] = {last + period, -rate8};
    }
    if (ec == 0) return 0;
    sort(ev, ev + ec, [](const Event& x, const Event& y) {
        return x.t < y.t || (x.t == y.t && x.d < y.d);
    });
    long long backlog8 = 0;
    int active = 0, prev = ev[0].t;
    for (int p = 0; p < ec; ) {
        const int t = ev[p].t;
        backlog8 += 1LL * (active - 8) * (t - prev);
        if (backlog8 < 0) backlog8 = 0;
        while (p < ec && ev[p].t == t) active += ev[p++].d;
        prev = t;
    }
    return max(0, prev + int((backlog8 + 7) / 8));
}

static ll mixed_free_objective(const vector<ll>& A, const MixedFreeState& s) {
    int d = 0;
    for (int t = 0; t < s.C; ++t) d = max(d, mixed_free_delay(A, s.dest, t));
    return max<ll>(s.err, d);
}

static void mixed_free_init(MixedFreeState& s, const vector<ll>& A,
    ...
    iota(order.begin(), order.end(), 0);
    for (int id = 0; id < s.N; ++id) s.value[id] = mixed_value(A, id);
    shuffle_ints(order, rng);
    ...
    for (int id : order) {
        ...
        ll bs = (1LL << 62);
        for (int t = 0; t < s.C; ++t) {
            ll score = llabs(s.sum[t] + s.value[id] - B[t])
                     - llabs(s.sum[t] - B[t]) + rng.mod(257);
            if (score < bs) bs = score, bd = t;
        }
        s.dest[id] = bd;
        s.sum[bd] += s.value[id];
    }
    s.err = 0;
    for (int t = 0; t < s.C; ++t) s.err += llabs(s.sum[t] - B[t]);
}

static void mixed_free_move(MixedFreeState& s, int id, int nd,
                            const vector<ll>& B) {
    const int od = s.dest[id];
    if (od == nd) return;
    const ll old = llabs(s.sum[od] - B[od]) + llabs(s.sum[nd] - B[nd]);
    s.sum[od] -= s.value[id];
    s.sum[nd] += s.value[id];
    s.dest[id] = nd;
    s.err += llabs(s.sum[od] - B[od]) + llabs(s.sum[nd] - B[nd]) - old;
}

static void insert_mixed_free(vector<MixedFreeAssignment>& out,
                              const MixedFreeState& s, ll score, int limit) {
    for (const auto& q : out) if (q.dst == s.dest) return;
    out.push_back({s.dest, score, s.err});
    sort(out.begin(), out.end(), [](const MixedFreeAssignment& x,
                                    const MixedFreeAssignment& y) {
        if (x.predicted != y.predicted) return x.predicted < y.predicted;
        return x.err < y.err;
    });
    if (int(out.size()) > limit) out.resize(limit);
}

static vector<MixedFreeAssignment> make_mixed_free_assignments(
        const vector<ll>& A, const vector<ll>& B,
        chrono::steady_clock::time_point end, uint32_t seed) {
    FastRNG rng(seed ^ 0xB7E15162u);
    vector<MixedFreeAssignment> out;
    out.reserve(40);
    while (chrono::steady_clock::now() < end) {
        MixedFreeState s;
        mixed_free_init(s, A, B, rng);
        ll current = mixed_free_objective(A, s);
        ll local = (1LL << 60);
        const int iterations = 90000;
        for (int it = 0; it < iterations; ++it) {
            if ((it & 63) == 0 && chrono::steady_clock::now() >= end) break;
            const int id = rng.mod(s.N), nd = rng.mod(s.C), od = s.dest[id];
            if (nd == od) continue;
            const ll before = current;
            mixed_free_move(s, id, nd, B);
            const ll after = mixed_free_objective(A, s);
            const double temp = 130000.0 * pow(0.0004, double(it) / iterations);
            bool accept = after <= before;
            if (!accept) accept = exp(double(before - after) / max(1.0, temp))
                                > (double(rng.next()) + .5) / 4294967296.0;
            if (accept) current = after;
            else mixed_free_move(s, id, od, B);
            if ((it & 63) == 63) {
                ll gain = 0;
                int bi = -1, bt = -1;
                for (int x = 0; x < s.N; ++x) {
                    const int from = s.dest[x];
                    for (int t = 0; t < s.C; ++t) if (t != from) {
                        const ll old = llabs(s.sum[from] - B[from]) + llabs(s.sum[t] - B[t]);
                        const ll nw = llabs(s.sum[from] - s.value[x] - B[from])
                                    + llabs(s.sum[t] + s.value[x] - B[t]);
                        if (nw - old < gain) gain = nw - old, bi = x, bt = t;
                    }
                }
                if (bi >= 0) {
                    mixed_free_move(s, bi, bt, B);
                    current = mixed_free_objective(A, s);
                }
            }
            if (current < local) {
                local = current;
                insert_mixed_free(out, s, current, 40);
            }
        }
    }
    return out;
...
static bool place_mixed_queue_integrated(const vector<ll>& A, int M,
                                         const vector<int>& dst, int R,
                                         uint32_t seed, const Deadline& deadline,
                                         PhaseState& out) {
    const int C = int(A.size());
    FastRNG rng(seed ^ 0xC0AC29B7u);
    static const int dr[4] = {-1, 1, 0, 0};
    static const int dc[4] = {0, 0, -1, 1};
    static const char ch[4] = {'U', 'D', 'L', 'R'};
    vector<int> order(C);
    iota(order.begin(), order.end(), 0);
    for (int restart = 0; restart < 600 && !deadline.expired(); ++restart) {
        PhaseState s{C, R, vector<uint16_t>(R),
                     vector<vector<string>>(R, vector<string>(C, "X")), {},
                     vector<int>(R * C, -1)};
        shuffle_ints(order, rng);
        ...
        for (int src : order) {
            if (A[src] == 0) continue;
            bool placed = false;
            for (int at = 0; at < 1800 && !deadline.expired() && !placed; ++at) {
                PhaseState q = s;
                const int rr = 2 + rng.mod(max(1, R - 3));
                const int cc = 1 + rng.mod(C - 2);
                if (cc != src && abs(cc - src) < 2) continue;
                ...
                int rd[4] = {0, 1, 2, 3};
                for (int z = 3; z > 0; --z) swap(rd[z], rd[rng.mod(z + 1)]);
                const int refined = rd[0];
                int cd[3], nc = 0;
                for (int d = 0; d < 4; ++d) if (d != (refined ^ 1)) cd[nc++] = d;
                for (int z = 2; z > 0; --z) swap(cd[z], cd[rng.mod(z + 1)]);
                const int cr = rr + dr[refined], cc2 = cc + dc[refined];
                bool ok = take(cr, cc2);
                string rt;
                for (int z = 0; z < 4; ++z) rt.push_back(ch[rd[z]]);
                q.g[rr][cc] = rt;
                vector<PhaseTerminal> term;
                for (int z = 1; z < 4 && ok; ++z) {
                    const int tr = rr + dr[rd[z]], tc = cc + dc[rd[z]];
                    if (!take(tr, tc)) { ok = false; break; }
                    const int item = z - 1;
                    if (mixed_value(A, 5 * src + item) > 0)
                        term.push_back({tr, tc, dst[5 * src + item], 4,
                            int((ll(M) - A[src] + 1 + z + root_delay) & 15)});
                }
                string ct;
                for (int z = 0; z < 2 && ok; ++z) {
                    ct.push_back(ch[cd[z]]);
                    const int tr = cr + dr[cd[z]], tc = cc2 + dc[cd[z]];
                    if (tr < 1 || tr >= R || tc < 0 || tc >= C || !take(tr, tc)) {
                        ok = false;
                        break;
                    }
                    const int item = 3 + z;
                    if (mixed_value(A, 5 * src + item) > 0)
                        term.push_back({tr, tc, dst[5 * src + item], 8,
                            int((ll(M) - A[src] + 1 + 4 * z + root_delay + 1) & 15)});
                }
                if (!ok) continue;
                q.g[cr][cc2] = ct;
                for (const PhaseTerminal& t : term) {
                    const string down = jump_token(R - t.r, 'D');
                    if (t.c == t.target) {
                        if (q.g[t.r][t.c] != "X" && q.g[t.r][t.c] != down) { ok = false; break; }
                        q.g[t.r][t.c] = down;
                        continue;
                    }
                    if (q.occ[t.r] >> t.target & 1) {
                        if (q.g[t.r][t.target] != down) { ok = false; break; }
                    } else {
                        phase_take(q, t.r, t.target);
                        q.g[t.r][t.target] = down;
                    }
                    q.g[t.r][t.c] = jump_token(abs(t.target - t.c),
                                                t.target > t.c ? 'R' : 'L');
                }
                if (!ok) continue;
                q.terms.insert(q.terms.end(), term.begin(), term.end());
                s = move(q);
                placed = true;
            }
            if (!placed) { all = false; break; }
...
static bool build_mixed_queue_board(const vector<ll>& A, int M,
                                    const vector<int>& dst, int R, uint32_t seed,
                                    const Deadline& deadline, Board& out) {
    PhaseState s;
    if (!place_mixed_queue_integrated(A, M, dst, R, seed, deadline, s)) return false;
    out.R = R;
    out.g = move(s.g);
    return true;
}
...
static void run_tail_general_search(const vector<ll>& A, const vector<ll>& B,
                                    int T, int M, uint32_t base_seed,
                                    chrono::steady_clock::time_point begin,
                                    chrono::steady_clock::time_point end,
                                    SimResult& best_result, Board& best_board);
...
int main() {
    ...
    const auto hard_end = begin + chrono::milliseconds(C >= 8 ? 1850 : 1780);
    ...
    const int initial_search_ms = C == 5 ? 115 : 105;
    const int initial_build_ms = C == 6 ? 245 : 330;
    vector<AssignmentK> legacy = make_strong_k4_assignments(A, B, begin + chrono::milliseconds(initial_search_ms), base_seed);
    auto merge_legacy = [&](vector<AssignmentK>&& add) {
        for (auto& q : add) {
            bool duplicate = false;
            for (const auto& x : legacy) if (x.dst == q.dst) { duplicate = true; break; }
            if (!duplicate) legacy.push_back(move(q));
        }
        sort(legacy.begin(), legacy.end(), [](const AssignmentK& x, const AssignmentK& y) {
            return x.err < y.err;
        });
        if (legacy.size() > 48) legacy.resize(48);
    };
    if (C == 5 && chrono::steady_clock::now() < begin + chrono::milliseconds(165))
        merge_legacy(make_strong_k4_assignments(A, B, begin + chrono::milliseconds(165), base_seed ^ 0x9E3779B9u));
    if (C == 5 && chrono::steady_clock::now() < begin + chrono::milliseconds(205)) {
        Deadline exact_deadline{begin + chrono::milliseconds(205)};
        merge_legacy(exact_k4_c5(A, B, exact_deadline));
    }
#ifdef LOCAL_DEBUG
    cerr << "LEGACY," << (legacy.empty() ? -1 : legacy[0].err) << '\n';
#endif
    ...
    if (C == 5) {
        for (int d : {2,3,4,1,5,6,0,7,8,9,10,11,12}) legacy_rows.push_back(C + d);
    } else {
        for (int d = 4; d <= 10; ++d) legacy_rows.push_back(C + d);
        for (int d = 0; d < 4; ++d) legacy_rows.push_back(C + d);
    }
    for (int ai = 0; ai < int(legacy.size()) && ai < (C == 5 ? 24 : 12) && !legacy_deadline.expired(); ++ai) {
        int built_rows = 0;
        for (int rr : legacy_rows) {
            ...
            bool found = false;
            for (int v = 0; v < (C == 5 ? 36 : 48) && !legacy_deadline.expired(); ++v) {
                Board cand;
            ...
            if (found && ++built_rows >= (C == 5 ? 3 : 1)) break;
        }
    }
    ...
    bool mixed_allowed = true;
    for (ll x : A) if (x == 0) mixed_allowed = false;
    if (C >= 8 && !legacy.empty() && legacy[0].err + 512 < best_result.cost &&
        chrono::steady_clock::now() + chrono::milliseconds(80) < hard_end) {
        vector<PendingBoard> phase4_pool;
        const auto phase4_end = min(hard_end, chrono::steady_clock::now() + chrono::milliseconds(210));
        Deadline phase4_deadline{phase4_end};
        for (int ai = 0; ai < int(legacy.size()) && ai < 5 && !phase4_deadline.expired(); ++ai) {
            for (int d = 7; d <= 11 && !phase4_deadline.expired(); ++d) {
                const ll lower = (1LL << d) + legacy[ai].err;
                if (lower >= best_result.cost) continue;
                for (int v = 0; v < 3 && !phase4_deadline.expired(); ++v) {
                    Board cand;
                    Deadline one{min(phase4_end, chrono::steady_clock::now() + chrono::milliseconds(36))};
                    uint32_t sd = base_seed ^ uint32_t(0x6A09E667u + ai * 10007u + d * 1009u + v * 65537u);
                    if (!build_phase_k4_board(A, M, legacy[ai].dst, C + d, sd, one, cand)) continue;
                    phase4_pool.push_back({lower, move(cand)});
                    break;
                }
            }
        }
        sort(phase4_pool.begin(), phase4_pool.end(), [](const PendingBoard& x, const PendingBoard& y) {
            return x.key < y.key;
        });
        for (int i = 0; i < int(phase4_pool.size()) && i < 3 && chrono::steady_clock::now() < hard_end; ++i) {
            SimResult r = simulate_board(A, B, T, M, phase4_pool[i].board);
            if (better_result(r, phase4_pool[i].board, best_result, best_board)) {
                best_result = r;
                best_board = move(phase4_pool[i].board);
            }
        }
    }
    if (mixed_allowed && C == 5 && best_result.cost > 3500 &&
        chrono::steady_clock::now() + chrono::milliseconds(150) < hard_end) {
        const auto free_begin = chrono::steady_clock::now();
        const int free_assign_ms = C == 6 ? 38 : 60;
        const int free_total_ms = C == 6 ? 85 : 185;
        const auto assign_end = min(hard_end, free_begin + chrono::milliseconds(free_assign_ms));
        vector<MixedFreeAssignment> free_mix = make_mixed_free_assignments(
                A, B, assign_end, base_seed ^ 0x9B05688Cu);
#ifdef LOCAL_DEBUG
        cerr << "FREE," << (free_mix.empty() ? -1 : free_mix[0].predicted) << ','
             << (free_mix.empty() ? -1 : free_mix[0].err) << '\n';
#endif
        vector<PendingBoard> free_pool;
        const auto pack_end = min(hard_end, free_begin + chrono::milliseconds(free_total_ms));
        vector<int> rows;
        const int center_d = max(5, 15 - C);
        for (int z = 0; z <= 10; ++z) {
            const int d = z == 0 ? center_d
                        : center_d + (z & 1 ? -(z + 1) / 2 : z / 2);
            if (d >= 2 && d <= 16) rows.push_back(d);
        }
        for (int ai = 0; ai < int(free_mix.size()) && ai < 7 && chrono::steady_clock::now() < pack_end; ++ai) {
            for (int d : rows) {
                if (chrono::steady_clock::now() >= pack_end) break;
                const ll lower = (1LL << d) + free_mix[ai].predicted;
                if (lower >= best_result.cost + 1500) continue;
                for (int v = 0; v < 3 && chrono::steady_clock::now() < pack_end; ++v) {
                    Board cand;
                    Deadline one{min(pack_end, chrono::steady_clock::now() + chrono::milliseconds(18))};
                    uint32_t sd = base_seed ^ uint32_t(0x5BE0CD19u + ai * 10007u + d * 1009u + v * 65537u);
                    if (!build_mixed_queue_board(A, M, free_mix[ai].dst, C + d, sd, one, cand)) continue;
                    free_pool.push_back({lower, move(cand)});
                    break;
                }
            }
        }
        sort(free_pool.begin(), free_pool.end(), [](const PendingBoard& x, const PendingBoard& y) {
            return x.key < y.key;
        });
#ifdef LOCAL_DEBUG
        cerr << "FREEPOOL," << free_pool.size() << '\n';
#endif
        for (int i = 0; i < int(free_pool.size()) && i < 4 && chrono::steady_clock::now() < hard_end; ++i) {
            SimResult r = simulate_board(A, B, T, M, free_pool[i].board);
#ifdef LOCAL_DEBUG
            cerr << "FREESIM," << free_pool[i].key << ',' << r.cost << ',' << r.E << ',' << r.D << ',' << r.L << ',' << free_pool[i].board.R << '\n';
#endif
            if (better_result(r, free_pool[i].board, best_result, best_board)) {
                best_result = r;
                best_board = move(free_pool[i].board);
            }
        }
    }
    if (mixed_allowed && C != 6 && best_result.cost > 3500 &&
        chrono::steady_clock::now() + chrono::milliseconds(180) < hard_end) {
        const auto mixed_begin = chrono::steady_clock::now();
        const auto assignment_end = min(hard_end, mixed_begin + chrono::milliseconds(80));
        vector<MixedAssignment> mixed = make_mixed_assignments(A, B, legacy, assignment_end,
                                                                base_seed ^ 0x510E527Fu);
#ifdef LOCAL_DEBUG
        cerr << "MIX," << (mixed.empty() ? -1 : mixed[0].err) << '\n';
#endif
        vector<PendingBoard> mixed_pool;
        const auto pack_end = min(hard_end, mixed_begin +
                chrono::milliseconds(C == 5 ? 680 : 430));
        vector<int> rows;
        const int first_d = max(7, 17 - C);
        const int center_d = C == 5 ? 13 : max(7, 20 - C);
        const int last_d = C == 5 ? 14 : min(18, first_d + 8);
        if (C == 5) {
            rows = {12, 13, 14};
        } else {
            rows.push_back(center_d);
            for (int d = first_d; d < center_d; ++d) rows.push_back(d);
            for (int d = center_d + 1; d <= last_d; ++d) rows.push_back(d);
        }
        if (!mixed.empty() && mixed[0].err * 2 + 512 < best_result.cost)
        for (int ai = 0; ai < int(mixed.size()) && ai < 10 && chrono::steady_clock::now() < pack_end; ++ai) {
            for (int d : rows) {
                if (chrono::steady_clock::now() >= pack_end) break;
                const ll lower = (1LL << d) + mixed[ai].err;
                if (lower >= best_result.cost) continue;
                if (C == 5 && lower > (mixed[0].err < 10000 ? 20000 : 30000)) continue;
                if (C == 7 && lower > 30000) continue;
                for (int v = 0; v < 2 && chrono::steady_clock::now() < pack_end; ++v) {
                    Board cand;
                    const int attempt_ms = C == 5 ? 100
                                         : d == center_d ? 55 : 24;
                    Deadline one{min(pack_end, chrono::steady_clock::now() + chrono::milliseconds(attempt_ms))};
                    const uint32_t sd = base_seed ^ uint32_t(0x1F83D9ABu + ai * 10007u + d * 1009u + v * 65537u);
                    if (!build_mixed_board(A, M, mixed[ai].dst, C + d, sd, one, cand)) continue;
                    mixed_pool.push_back({lower, move(cand)});
                    break;
                }
            }
        }
        sort(mixed_pool.begin(), mixed_pool.end(), [](const PendingBoard& x, const PendingBoard& y) {
            return x.key < y.key;
        });
        for (int i = 0; i < int(mixed_pool.size()) && i < 4 && chrono::steady_clock::now() < hard_end; ++i) {
            SimResult r = simulate_board(A, B, T, M, mixed_pool[i].board);
            if (better_result(r, mixed_pool[i].board, best_result, best_board)) {
                best_result = r;
                best_board = move(mixed_pool[i].board);
            }
        }
    }
    ...
    if (false && (C == 8 || C == 10) && best_result.cost > 10000 && !legacy.empty() &&
    ...
    if (false && C == 5 && M >= 479292 && M <= 539215 && best_result.cost > 120000 && chrono::steady_clock::now() + chrono::milliseconds(260) < hard_end) {
    ...
    if (false && C == 9 && best_result.cost > 10000 && chrono::steady_clock::now() + chrono::milliseconds(180) < hard_end) {
        const auto phase_begin = chrono::steady_clock::now();
    ...
    if (best_result.cost <= 100000 && chrono::steady_clock::now() + chrono::milliseconds(130) < hard_end) {
    ...
    run_tail_general_search(A, B, T, M, base_seed, begin,
                            begin + chrono::milliseconds(1940),
                            best_result, best_board);
#ifdef LOCAL_DEBUG
    cerr<<"BEST,"<<best_result.cost<<","<<best_result.E<<","<<best_result.D<<","<<best_result.L<<","<<best_board.R<<"\n";
#endif
    cout<<best_board.R<<"\n";for(int r=0;r<best_board.R;r++){for(int c=0;c<C;c++){if(c)cout<<" ";cout<<best_board.g[r][c];}cout<<"\n";}return 0;
}

struct FreeQueueAssignment {
    vector<int> dst;
    ll predicted = (1LL << 60);
    ll err = (1LL << 60);
};
...
static void free_init(FreeState& s, const vector<ll>& A, const vector<ll>& B,
                      FastRNG& rng) {
    s.C = int(A.size());
    s.N = 4 * s.C;
    s.value.resize(s.N);
    for (int i = 0; i < s.C; ++i) for (int k = 0; k < 4; ++k)
        s.value[4 * i + k] = int((A[i] + 3 - k) / 4);
    s.dest.assign(s.N, -1);
    s.sum.assign(s.C, 0);
    vector<int> order(s.N);
    ...
    for (int item : order) {
        ...
        ll bs = (1LL << 62);
        for (int d = 0; d < s.C; ++d) {
            ll z = llabs(s.sum[d] + s.value[item] - B[d])
                 - llabs(s.sum[d] - B[d]) + int(rng.next() & 255u);
            if (z < bs) bs = z, bd = d;
        }
        s.dest[item] = bd;
        s.sum[bd] += s.value[item];
    }
    s.err = 0;
    for (int d = 0; d < s.C; ++d) s.err += llabs(s.sum[d] - B[d]);
}

static inline void free_move(FreeState& s, int item, int nd,
                             const vector<ll>& B) {
    int od = s.dest[item];
    if (od == nd) return;
    ll old = llabs(s.sum[od] - B[od]) + llabs(s.sum[nd] - B[nd]);
    s.sum[od] -= s.value[item];
    s.sum[nd] += s.value[item];
    s.dest[item] = nd;
    s.err += llabs(s.sum[od] - B[od]) + llabs(s.sum[nd] - B[nd]) - old;
...
static int estimate_queue_delay_free(const vector<ll>& A,
                                     const vector<int>& dest, int target) {
    struct Event { int t, delta; };
    Event ev[80];
    int ec = 0;
    const int C = int(A.size());
    for (int id = 0; id < 4 * C; ++id) if (dest[id] == target) {
        const int src = id >> 2, k = id & 3;
        const int cnt = int((A[src] + 3 - k) >> 2);
        if (cnt <= 0) continue;
        const int first4 = 4 * (1 - int(A[src])) + 4 * k;
        const int last4 = first4 + 16 * (cnt - 1);
        ev[ec++] = {first4, 1};
        ev[ec++] = {last4 + 16, -1};
    }
    if (ec == 0) return 0;
    sort(ev, ev + ec, [](const Event& a, const Event& b) {
        return a.t < b.t || (a.t == b.t && a.delta < b.delta);
    });
    long long backlog16 = 0;
    int active = 0, prev = ev[0].t;
    for (int p = 0; p < ec; ) {
        const int t = ev[p].t;
        backlog16 += 1LL * (active - 4) * (t - prev);
        if (backlog16 < 0) backlog16 = 0;
        while (p < ec && ev[p].t == t) active += ev[p++].delta;
        prev = t;
    }
    return max(0, (prev + 3) / 4 + int((backlog16 + 15) / 16));
}

static ll free_balanced_objective(const vector<ll>& A, const FreeState& s) {
    int d = 0;
    for (int j = 0; j < s.C; ++j)
        d = max(d, estimate_queue_delay_free(A, s.dest, j));
    return max<ll>(s.err, d);
}

static void insert_free_assignment(vector<FreeQueueAssignment>& out,
                                   const FreeState& s, ll predicted, int limit) {
    for (const auto& x : out) if (x.dst == s.dest) return;
    out.push_back({s.dest, predicted, s.err});
...
static vector<FreeQueueAssignment> make_free_queue_assignments(
        const vector<ll>& A, const vector<ll>& B,
        chrono::steady_clock::time_point local_end, uint32_t seed) {
    FastRNG rng(seed ^ 0x94D049BBu);
    vector<FreeQueueAssignment> out;
    out.reserve(32);
    while (chrono::steady_clock::now() < local_end) {
        FreeState s;
        free_init(s, A, B, rng);
        ll cur = free_balanced_objective(A, s);
        ll best_seen = (1LL << 60);
        const int iterations = 70000;
        for (int it = 0; it < iterations; ++it) {
            if ((it & 63) == 0 && chrono::steady_clock::now() >= local_end) break;
            int item = rng.mod(s.N), nd = rng.mod(s.C), od = s.dest[item];
            if (nd == od) continue;
            ll before = cur;
            free_move(s, item, nd, B);
            ll after = free_balanced_objective(A, s);
            double temp = 150000.0 * pow(0.0005, double(it) / iterations);
            bool accept = after <= before;
            if (!accept) accept = exp(double(before - after) / max(1.0, temp))
                                > (double(rng.next()) + .5) / 4294967296.0;
            if (accept) cur = after;
            else free_move(s, item, od, B);
            if ((it & 31) == 31) {
                ll bd = 0;
                int bi = -1, bt = -1;
                for (int x = 0; x < s.N; ++x) {
                    int from = s.dest[x];
                    for (int t = 0; t < s.C; ++t) if (t != from) {
                        ll old = llabs(s.sum[from] - B[from]) + llabs(s.sum[t] - B[t]);
                        ll nw = llabs(s.sum[from] - s.value[x] - B[from])
                              + llabs(s.sum[t] + s.value[x] - B[t]);
                        if (nw - old < bd) bd = nw - old, bi = x, bt = t;
                    }
                }
                if (bi >= 0) {
                    free_move(s, bi, bt, B);
                    cur = free_balanced_objective(A, s);
                }
            }
            if (cur < best_seen) {
                best_seen = cur;
                insert_free_assignment(out, s, cur, 32);
            }
        }
    }
    return out;
...
static bool vertical_token(const string& s) {
    if (s == "D") return true;
    if (s.size() < 2 || s.back() != 'D') return false;
    for (int i = 0; i + 1 < int(s.size()); ++i)
        if (!isdigit(static_cast<unsigned char>(s[i]))) return false;
...
struct GadgetCandidate {
    vector<pair<int,string>> cell;
    int score = 0;
};

static bool add_gadget_cell(vector<pair<int,string>>& cells, int id,
                            const string& token) {
    for (auto& z : cells) if (z.first == id)
        return z.second == token && vertical_token(token);
    cells.push_back({id, token});
    return true;
}

static bool build_csp_queue_board(const vector<ll>& A, const vector<int>& dst,
                                  int R, uint32_t seed,
                                  chrono::steady_clock::time_point end,
                                  Board& out) {
    const int C = int(A.size());
    ...
    static const int dc[4] = {0,0,-1,1};
    static const char ch[4] = {'U','D','L','R'};
    vector<vector<GadgetCandidate>> cand(C);
    for (int src = 0; src < C; ++src) {
        if (A[src] == 0) {
            cand[src].push_back({{},0});
            continue;
        }
        vector<int> centers;
        for (int c = 1; c + 1 < C; ++c) {
            int cnt = 0;
            for (int k = 0; k < 4; ++k) cnt += dst[4 * src + k] == c;
            if (cnt <= 2) centers.push_back(c);
        }
        if (centers.empty()) return false;
        const int attempts = C <= 7 ? 5000 : 3500;
        for (int it = 0; it < attempts && chrono::steady_clock::now() < end; ++it) {
            int center = centers[rng.mod(int(centers.size()))];
            int root = 2 + rng.mod(max(1, R - 3));
            if (root + 1 >= R) continue;
            int pm[4] = {0,1,2,3};
            for (int z = 3; z > 0; --z) swap(pm[z], pm[rng.mod(z + 1)]);
            vector<pair<int,string>> cells;
            bool ok = true;
            auto put = [&](int r, int c, const string& token) {
                if (r < 0 || r >= R || c < 0 || c >= C) return false;
                return add_gadget_cell(cells, r * C + c, token);
            };
            int mode = rng.mod(3);
            if (center == src) mode = 0;
            if (mode == 0) {
                if (!put(0, src, jump_token(root, 'D'))) ok = false;
                if (ok && center != src && !put(root, src,
                    jump_token(abs(center-src), center > src ? 'R' : 'L'))) ok = false;
            } else {
                int entry = mode == 1 ? 1 + rng.mod(max(1, root - 1))
                                      : max(1, root / 2);
                if (entry >= root) continue;
                if (!put(0, src, jump_token(entry, 'D'))) ok = false;
                if (ok && !put(entry, src,
                    jump_token(abs(center-src), center > src ? 'R' : 'L'))) ok = false;
                if (ok && !put(entry, center, jump_token(root-entry, 'D'))) ok = false;
            }
            string rt;
            for (int k = 0; k < 4 && ok; ++k) {
                int d = pm[k];
                int lr = root + dr[d], lc = center + dc[d];
                int target = dst[4 * src + k];
                if (lr <= 0 || lr >= R || lc < 0 || lc >= C) { ok = false; break; }
                if (lc == target) {
                    if (!put(lr, lc, jump_token(R-lr, 'D'))) ok = false;
                } else {
                    if (!put(lr, lc, jump_token(abs(target-lc), target > lc ? 'R' : 'L'))) ok = false;
                    if (ok && !put(lr, target, jump_token(R-lr, 'D'))) ok = false;
                }
                rt.push_back(ch[d]);
            }
            if (ok && !put(root, center, rt)) ok = false;
            if (!ok) continue;
            int score = int(cells.size()) * 12 + abs(center-src);
            for (auto& z : cells) if (!vertical_token(z.second)) score += 2;
            sort(cells.begin(), cells.end(), [](auto& x, auto& y) {
                return x.first < y.first;
            });
            bool duplicate = false;
            for (auto& old : cand[src]) if (old.cell == cells) { duplicate = true; break; }
            if (!duplicate) cand[src].push_back({move(cells),score});
            if (cand[src].size() >= 180) break;
        }
        if (cand[src].empty()) return false;
        sort(cand[src].begin(), cand[src].end(), [](const auto& x, const auto& y) {
            return x.score < y.score;
        });
    }
    vector<string> board(R * C, "X");
    vector<uint8_t> refs(R * C, 0), used(C, 0);
    int nodes = 0;
    function<bool(int)> dfs = [&](int dep) {
        if ((++nodes & 255) == 0 && chrono::steady_clock::now() >= end) return false;
        if (dep == C) return true;
        int src = -1, best_count = INT_MAX;
        for (int s = 0; s < C; ++s) if (!used[s]) {
            int count = 0;
            for (const auto& q : cand[s]) {
                bool ok = true;
                for (auto& z : q.cell) if (refs[z.first] && board[z.first] != z.second) {
                    ok = false;
                    break;
                }
                if (ok && ++count >= best_count) break;
            }
            if (count < best_count) best_count = count, src = s;
            if (count == 0) break;
        }
        if (src < 0 || best_count == 0) return false;
        used[src] = 1;
        int start = cand[src].empty() ? 0 : rng.mod(min<int>(7, cand[src].size()));
        int limit = min<int>(cand[src].size(), C <= 7 ? 180 : 120);
        for (int zz = 0; zz < limit; ++zz) {
            int qi = (start + zz) % int(cand[src].size());
            const auto& q = cand[src][qi];
            bool ok = true;
            for (auto& z : q.cell) if (refs[z.first] && board[z.first] != z.second) {
                ok = false;
                break;
            ...
            if (!ok) continue;
            for (auto& z : q.cell) {
                if (!refs[z.first]) board[z.first] = z.second;
                ++refs[z.first];
            }
            if (dfs(dep + 1)) return true;
            for (auto& z : q.cell) if (--refs[z.first] == 0) board[z.first] = "X";
    ...
    out.R = R;
    out.g.assign(R, vector<string>(C));
    for (int r = 0; r < R; ++r) for (int c = 0; c < C; ++c)
        out.g[r][c] = board[r * C + c];
    return true;
}
...
static void run_tail_general_search(const vector<ll>& A, const vector<ll>& B,
    ...
    if (C > 7 || best_result.cost <= 10000 ||
        now + chrono::milliseconds(90) >= end) return;
    auto assign_end = min(end, now + chrono::milliseconds(260));
    vector<FreeQueueAssignment> fq = make_free_queue_assignments(
            A, B, assign_end, base_seed ^ 0x6A09E667u);
    if (fq.empty() || chrono::steady_clock::now() + chrono::milliseconds(50) >= end) return;
    struct TailCandidate { ll key; Board board; };
    vector<TailCandidate> pool;
    auto pack_end = min(end, chrono::steady_clock::now() + chrono::milliseconds(300));
    int topn = min<int>(10, fq.size());
    for (int qi = 0; qi < topn && chrono::steady_clock::now() < pack_end; ++qi) {
        if (fq[qi].predicted + 8 >= best_result.cost + 12000) continue;
        for (int d = 3; d <= 14 && chrono::steady_clock::now() < pack_end; ++d) {
            int R = C + d;
            ll lower = (1LL << d) + fq[qi].predicted;
            if (lower >= best_result.cost + 7000) continue;
            for (int v = 0; v < 3 && chrono::steady_clock::now() < pack_end; ++v) {
                Board b;
                auto one_end = min(pack_end, chrono::steady_clock::now() + chrono::milliseconds(32));
                uint32_t sd = base_seed ^ uint32_t(0xBB67AE85u + qi * 1009u + R * 9176u + v * 65537u);
                if (build_csp_queue_board(A, fq[qi].dst, R, sd, one_end, b)) {
                    pool.push_back({lower, move(b)});
                    break;
                }
            }
        }
    }
    sort(pool.begin(), pool.end(), [](const TailCandidate& x,
                                      const TailCandidate& y) {
        return x.key < y.key;
    });
    int checked = 0;
    for (int i = 0; i < int(pool.size()) &&
         chrono::steady_clock::now() + chrono::milliseconds(15) < end; ++i) {
        SimResult r = simulate_board(A, B, T, M, pool[i].board);
        if (better_result(r, pool[i].board, best_result, best_board)) {
            best_result = r;
            best_board = move(pool[i].board);
